...
function handleEdit(row) {
    router.get(route('admin.feedback.template.edit', {template: row.id}))
}
function handleDeleteEntity(row) {
    $delete_entity.show(route('admin.feedback.template.destroy', {template: row.id}));
}
function createButton() {
    router.get(route('admin.feedback.template.create'))
}
function routeClick(row) {
    router.get(route('admin.feedback.template.show', {template: row.id}))
}
function handleToggle(index, row) {
    router.visit(
        route('admin.feedback.template.toggle', {template: row.id}),
        {
        method: 'post'
    });
}
