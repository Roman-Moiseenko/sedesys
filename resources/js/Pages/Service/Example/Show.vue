<template>
            ...
            <div class="p-4">
                <el-descriptions :column="2" border>
                    ...
                    <el-descriptions-item label="Персонал">
                        <el-tag type="info" class="font-medium ml-6" v-for="item in example.employees">
    ...
    <div class="mt-5 p-5 bg-white rounded-lg">
        ...
        <el-upload
            ...
            :action="route('admin.service.example.add', {example: props.example.id})"
...
const handleRemove: UploadProps['onRemove'] = (uploadFile, uploadFiles) => {
    if (uploadFile.id !== undefined)
        router.post(
            route('admin.service.example.del', {example: props.example.id}),
            {photo_id: uploadFile.id}
        );
}
...
function onSubmit() {
    ...
    router.post(route('admin.service.example.set', {example: props.example.id}), form);
}
function handleEdit() {
    router.get(route('admin.service.example.edit', {example: props.example.id}))
}
function handleToggle() {
    router.post(route('admin.service.example.toggle', {example: props.example.id}));
}
</script>
