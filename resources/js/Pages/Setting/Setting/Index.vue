<template>
    ...
    <el-config-provider :locale="ru">
        <h1 class="font-medium text-xl">Setting</h1>
        <el-button type="primary" class="p-4 my-3" @click="createButton">Добавить Setting</el-button>
    ...
    <el-dialog v-model="$data.dialogDelete" title="Удалить запись" width="400" center>
        <div class="font-medium text-md mt-2">
            Вы уверены, что хотите удалить setting?
        </div>
...
export default {
    ...
    methods: {
        createButton() {
            router.get('/admin/setting/setting/create')
        },
        routeClick(row) {
            router.get(row.url)
        },
